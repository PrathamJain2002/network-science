...
class VideoTranslator:
    ...
    def __init__(
        ...
        self.audio_separator = AudioSeparator(method="custom", logger=self.logger)
    ...
    def extract_video_frames(self, video_path: str, output_dir: str, test_mode: bool = False) -> bool:
        ...
        try:
            ...
            # Configure frame extraction based on mode
            if test_mode:
                # Test mode: extract fewer frames for faster processing
                fps = 'fps=1/2'  # 1 frame every 2 seconds
                quality = '3'    # Lower quality for testing
                max_frames = 5   # Limit to 5 frames max
            else:
                # Production mode: normal frame extraction
                fps = 'fps=30'   # Extract at 30 FPS
            ...
            # Add frame limit for test mode
            if test_mode and max_frames:
            ...
            # Add frame limit for test mode
            if test_mode and max_frames:
                hw_cmd.insert(-4, '-frames:v')
                hw_cmd.insert(-4, str(max_frames))
    ...
    def separate_audio_tracks(self, audio_path: str, output_dir: str) -> Dict[str, str]:
        """
        ...
        try:
            if self.logger:
                self.logger.info(f"🎵 Separating audio tracks...")
            
            # Ensure output directory exists
            os.makedirs(output_dir, exist_ok=True)
            
            # Perform audio separation
            result = self.audio_separator.separate_audio(audio_path, output_dir)
            
            # Analyze separation quality
            quality = self.audio_separator.get_separation_quality(
                audio_path, result['voice'], result['music']
            )
            
            if self.logger:
                self.logger.info(f"📊 Audio separation quality: {quality['overall_quality']:.1f}/100")
                self.logger.info(f"🎤 Voice track: {result['voice']}")
                self.logger.info(f"🎵 Music track: {result['music']}")
...
"""
    ...
    def create_final_video(self, frames_pattern: str, voice_audio: str, 
                          music_audio: str, output_file: str, total_frames: int) -> bool:
        """Create final video with synchronized voice and background music using optimized FFmpeg"""
        try:
            if self.logger:
                self.logger.info(f"🎬 Creating final video with voice and background music...")
            
            # Use voice audio duration for synchronization
            audio_duration = self.get_audio_duration(voice_audio)
            ...
            # Optimized FFmpeg command for faster encoding with mixed audio
            cmd = [
                'ffmpeg',
                '-threads', '0',  # Use all available CPU threads
                '-framerate', f'{required_fps:.2f}',
                '-i', frames_pattern,
                '-i', voice_audio,  # Marathi voice track
                '-i', music_audio,  # Background music track
                ...
                # Audio mixing and encoding optimization  
                '-filter_complex', '[1:a][2:a]amix=inputs=2:duration=first:dropout_transition=2',  # Mix voice and music
                '-c:a', 'aac',
                '-b:a', '192k',  # Higher bitrate for mixed audio
                '-ar', '44100',  # Standard sample rate
            ...
            hw_cmd = [
                ...
                # Audio mixing (same as before)
                '-filter_complex', '[1:a][2:a]amix=inputs=2:duration=first:dropout_transition=2',
                '-c:a', 'aac',
                '-b:a', '192k',
                '-ar', '44100',
    ...
    def process_video(self, video_path: str, video_id: Optional[str] = None, cleanup: bool = True, moving_text: bool = False, advanced_ocr: bool = True, test_mode: bool = False) -> str:
        """
        ...
        Args:
            ...
            advanced_ocr: Whether to use advanced OCR with image preprocessing and coordinate scaling
            test_mode: Whether to use test mode (faster processing with fewer frames)
        ...
        separated_audio_dir = os.path.join("separated_audio", video_id)
        ...
        temp_dirs = [frames_dir, translated_frames_dir, separated_audio_dir]
        temp_files = [audio_file, marathi_audio_file]
        
        try:
            # Step 1: Extract frames
            print("Step 1: Extracting video frames...")
            if not self.extract_video_frames(video_path, frames_dir, test_mode=test_mode):
            ...
            # Step 2.5: Separate audio tracks (voice + background music)
            print("Step 2.5: Separating audio tracks...")
            separated_tracks = self.separate_audio_tracks(audio_file, separated_audio_dir)
            voice_track = separated_tracks['voice']
            music_track = separated_tracks['music']
            ...
            # Step 6: Transcribe and translate audio (using separated voice track)
            print("Step 6: Processing audio...")
            marathi_text = self.transcribe_and_translate_audio(voice_track, text_buffer)
            if not marathi_text:
            ...
            if not self.create_final_video(frames_pattern, marathi_audio_file, music_track, output_video, total_frames):
